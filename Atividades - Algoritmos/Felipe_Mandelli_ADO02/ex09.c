...
//          Cada elemento de s[] na posicao i contem a 
//          soma dos prefixos do vetor v[] nessa posicao i.
//          Logo, s e definida por: 
//          1. s[0] = v[0];
//          2. s[i] = v[i] + v[i-1] + v[i-2] + ... + v[0]  
//[]-------------------------------------------------------------[]
float soma_dos_prefixos(int v[], int s[], int n){

	s[0] = v[0];

	for (int i = 1; i < n; i++){
	
		for (int controle = i; controle >= 0; controle--){
			s[i] = s[i] + v[controle];
		}
